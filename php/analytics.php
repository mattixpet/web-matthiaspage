<?php
  ...
  $errors = array();
  try {
    // Connection to Dream of Songs database
    $dosconn = new PDO(/* here is some private information about connecting to the dream of songs database! */);
    $dosconn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);

    // Connection to web page stats for the normal web page
    $webconn = new PDO(/* here is some private information about connecting to the web page stats database! */);
    $webconn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
  ...
  if (sizeof($errors) === 0) {
    ...
    if ($method === 'POST') {
      ...
      switch ($data['type']) {
        // pageload is from the game only, legacy, it used to be the only site
        case 'pageload':
        // webpageload is from the page itself, e.g. works, /, etc.
        case 'webpageload':
          $conn = $data['type'] == 'pageload' ? $dosconn : $webconn;
          ...
          if ($data['type'] == 'webpageload') {
            $query = $conn->prepare(
              'INSERT INTO pages (url, count) ' .
              'VALUES (:url, count) ' .
              'ON DUPLICATE KEY UPDATE count = count + 1'
            );
            $results[] = $query->execute(array('url' => $data['url']));
          }
        ...
        case 'milestone':
          ...
          $query = $dosconn->prepare('UPDATE general_stats SET count = count + 1 WHERE label LIKE :value');
